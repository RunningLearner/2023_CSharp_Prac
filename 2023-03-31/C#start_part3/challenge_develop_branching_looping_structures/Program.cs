...
do
{
    ...
    switch (menuSelection)
    {
        ...
        case "3":
            // 2023.03.31 작성 Ensure animal ages and physical descriptions are complete 
            for (int i = 0; i < maxPets; i++)
            {
                if (ourAnimals[i, 0] != "ID #: ")
                {   
                    ...
                    //숫자로 변환 가능한지
                    if (!int.TryParse(ourAnimals[i, 2].Split(" ")[1], out number))
                    {   
                        Console.WriteLine(ourAnimals[i, 2], number);
                        Console.WriteLine(ourAnimals[i, 0]);
                        Console.WriteLine("Please edit Age\n");

                        readResult = Console.ReadLine();
                        while(!int.TryParse(readResult, out number))
                        {
                            Console.WriteLine("Please edit Age\n");
                            readResult = Console.ReadLine();
                        }
                    ...
                    //외형 설명이 비었으면 채우기
                    if (ourAnimals[i, 4] == "tbd" || ourAnimals[i, 4] == null || ourAnimals[i, 4].Length == 22)
                    {
                        Console.WriteLine(ourAnimals[i, 0]);
